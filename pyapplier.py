    ...
    with open(filepath) as tsv:
      ...
      for line in csv.reader(tsv, dialect="excel-tab"):
        ...
        if line[5] == 'S':
          skipped = skipped + 1
          if dry == False:
            print ('[X] Skipped ({}%): Artist: {}; Track: {}; Time: {}'.format(str(round(100*(iteration-3)/(tracks-3),2)),artist,track,datetime.datetime.fromtimestamp(int(unix_timestamp)).strftime('%Y-%m-%d %H:%M:%S')))
          continue
        elif line[5] != 'L':
